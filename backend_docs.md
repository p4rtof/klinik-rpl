...
### ERD Saat Ini (Current ERD)
Berikut adalah gambaran relasi ERD berdasarkan struktur tabel aktif pada database `schema.prisma`.

```mermaid
erDiagram
    User ||--o{ Jadwal : "memiliki"
    User ||--o{ RekamMedis : "membuat"
    
    Pasien ||--o{ Jadwal : "mendaftar"
    Pasien ||--o{ RekamMedis : "memiliki"
    Pasien ||--o{ Pembayaran : "melakukan"

    Jadwal ||--o{ RekamMedis : "menghasilkan"

    RekamMedis ||--o{ Diagnosis : "memiliki"
    RekamMedis ||--o{ Resep : "diberikan"
    RekamMedis ||--o| Rujukan : "menerbitkan"
    RekamMedis ||--o{ Pembayaran : "ditagihkan"
    ...
    RekamMedis {
        ...
        String keluhan
        String tindakan
        String anamnesisKeluhanUtama
        Int tdSistolik
        Int tdDiastolik
        String edukasiPasien
        String catatanTambahan
    }

    Diagnosis {
        String id PK
        String rekamMedisId FK
        String diagnosis
        DateTime createdAt
    }
...
### Rekomendasi ERD Disempurnakan (Improved ERD v2)
Berikut adalah usulan struktur ERD masa depan (V2) yang lebih ternormalisasi dan ketat, dilengkapi dengan `Obat`, `Penyakit` (ICD-10), serta *Hard Limit Constraints*.

```mermaid
erDiagram
    %% Relasi User (Admin & Dokter) ke Jadwal
    User ||--o{ Jadwal : "menangani (Admin/Dokter)"
    User ||--o{ RekamMedis : "memeriksa (Dokter)"
    ...
    %% Relasi 1 to 1
    Jadwal ||--o| RekamMedis : "menghasilkan (1 to 1)"
    RekamMedis ||--o| Pembayaran : "ditagihkan (1 to 1)"
    RekamMedis ||--o| Rujukan : "menerbitkan (1 to 1)"

    RekamMedis ||--o{ Diagnosis : "ditegakkan"
    RekamMedis ||--o{ Resep : "diberikan"

    %% Tabel Obat & Penyakit
    Penyakit ||--o{ Diagnosis : "diklasifikasikan"
    Obat ||--o{ Resep : "menggunakan"

    User {
        String id PK
        String role "ADMIN / DOKTER"
        String username
        String namaLengkap
        String spesialisasi
    }
    ...
    Pasien {
        ...
        String noTelepon "NUMERIC ONLY, MAX: 15"
        String alamat
    }

    Obat {
        String id PK
        String kodeObat "UNIQUE"
        String namaObat
        Float harga
        Int stok
    }
    ...
    Jadwal {
        String id PK
        String pasienId FK
        String dokterId FK "Dokter yg bertugas"
        DateTime tanggal
    ...
    RekamMedis {
        ...
        String pasienId FK
        String dokterId FK
        String jadwalId FK "UNIQUE (1:1)"
        DateTime tanggal
        String keluhan
        String anamnesisRps
        Int tdSistolik "LIMIT: 0-300"
        Int tdDiastolik "LIMIT: 0-200"
        Float bb "LIMIT: 0-300 kg"
        Float tb "LIMIT: 0-250 cm"
        Float suhu "LIMIT: 30-45 C"
        Int rr "LIMIT: 0-100"
        Int nadi "LIMIT: 0-300"
    }

    Diagnosis {
        String id PK
        String rekamMedisId FK
        String penyakitId FK
        String catatanTambahan
    }
    ...
    Pembayaran {
        String id PK
        String pasienId FK
        String rekamMedisId FK "UNIQUE (1:1)"
        Float totalTagihan
        String status
    }
```
...
### 24 Mei 2026
- **Zod Validation Hard Limits**: Pengetatan constraint pada sisi backend untuk mengamankan integritas data medis.
  - `noTelepon` kini dibatasi maksimal 15 digit dan wajib hanya mengandung angka (Regex).
  - `jam` kunjungan wajib berformat standar `HH:MM` (Regex).
  - Semua indikator Tanda-Tanda Vital (TTV) pada Rekam Medis (seperti Suhu, Sistolik, Diastolik, BB, TB) telah diberikan *Hard Limit* (.min dan .max) untuk menghindari *fat-finger error* saat pengisian data.
