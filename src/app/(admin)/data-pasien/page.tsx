          ...
          <tbody className="divide-y divide-gray-100 text-center font-semibold">
            {isTableLoading ? (
              ...
              pasienList.map((pasien: any) => (
                <tr key={pasien.id} className="hover:bg-blue-50/50 transition-colors text-lg h-fit">
                  <td className="p-2 text-primary font-bold">{pasien.id || "-"}</td>
                  <td className="p-2 text-left capitalize">{pasien.nama}</td>
                  <td className="p-2">{pasien.tanggalLahir ? new Date(pasien.tanggalLahir).toLocaleDateString("id-ID", { day: "2-digit", month: "long", year: "numeric" }) : "-"}</td>
                  <td className="p-2">{pasien.jenisKelamin === "LAKI_LAKI" ? "Laki-laki" : "Perempuan"}</td>
                  <td className="p-2">{pasien.noTelepon || "-"}</td>
