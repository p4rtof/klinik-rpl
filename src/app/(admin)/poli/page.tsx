...
export default function PoliPage() {
  ...
  const fetchPoli = async () => {
    ...
    try {
      ...
      if (json.success) {
        setPoliList(json.data);
      }
    } catch (err) {
      console.error(err);
      showNotification("error", "Gagal mengambil data poliklinik.");
    } finally {
  ...
  const filteredList = useMemo(() => {
    return poliList.filter((item) =>
      item.namaPoli.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [poliList, searchQuery]);
  ...
  const handleOpenEdit = (poli: Poli) => {
    setFormData({
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const url = isEditing ? `/api/poli/${selectedId}` : "/api/poli";
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          namaPoli: formData.namaPoli.trim(),
      ...
      const json = await res.json();
      if (json.success) {
        showNotification("success", isEditing ? "Poliklinik berhasil diperbarui." : "Poliklinik berhasil ditambahkan.");
        setIsFormOpen(false);
        fetchPoli();
      } else {
  ...
  const handleDeleteExecute = async () => {
    ...
    try {
      const res = await fetch(`/api/poli/${deleteTargetId}`, {
        method: "DELETE",
        headers: { "x-user-role": "ADMIN" },
      });
      const json = await res.json();
      if (json.success) {
        showNotification("success", "Poliklinik berhasil dihapus.");
        fetchPoli();
      } else {
        showNotification("error", json.error || "Gagal menghapus poliklinik.");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-primary">Kelola Data Poliklinik</h1>
          <p className="text-gray-500 mt-1">Daftar poli pelayanan medis dan keterangannya</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead className="bg-primary text-white">
            <tr>
              <th className="px-6 py-3 text-lg font-bold border-r border-2 border-white/50 text-center uppercase">Nama Poliklinik</th>
              <th className="px-6 py-3 text-lg font-bold border-r border-2 border-white/50 text-center uppercase">Keterangan / Deskripsi Layanan</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {isTableLoading ? (
              <tr>
                <td colSpan={3} className="p-10 text-center text-gray-500">Memuat data poliklinik...</td>
              </tr>
            ) : paginatedList.length > 0 ? (
              paginatedList.map((item) => (
                <tr key={item.id} className="hover:bg-blue-50 transition-colors text-center text-lg font-semibold">
                  <td className="px-6 py-4 font-bold text-primary text-left w-[30%] capitalize">{item.namaPoli}</td>
                  <td className="px-6 py-4 text-left font-medium text-gray-700">{item.keterangan || "-"}</td>
              ...
              <tr>
                <td colSpan={3} className="px-6 py-10 text-center text-gray-400 italic">Belum ada data poliklinik.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="bg-primary text-white p-5 text-center">
              <h2 className="text-xl font-black">{isEditing ? "Edit Poliklinik" : "Tambah Poli Baru"}</h2>
            </div>
            <form onSubmit={handleFormSubmit} className="p-6 space-y-4">
              <div>
                <label className="text-sm font-bold text-gray-600 mb-1 block">Nama Poliklinik</label>
                <input
                  type="text"
                  required
                  value={formData.namaPoli}
                  onChange={(e) => setFormData({ ...formData, namaPoli: e.target.value })}
                  placeholder="Nama Poliklinik (misal: Poli Umum, Poli Gigi)"
                  className="w-full border-2 border-gray-200 p-3 rounded-xl outline-none font-bold text-gray-700 focus:border-primary"
                />
              </div>
              <div>
                <label className="text-sm font-bold text-gray-600 mb-1 block">Keterangan / Deskripsi</label>
                <textarea
                  value={formData.keterangan}
                  onChange={(e) => setFormData({ ...formData, keterangan: e.target.value })}
                  placeholder="Deskripsi singkat mengenai layanan poli..."
                  rows={4}
                  className="w-full border-2 border-gray-200 p-3 rounded-xl outline-none font-bold text-gray-700 focus:border-primary resize-none"
                />
              </div>
          ...
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              <p className="text-lg font-bold text-gray-700 text-center">
                Apakah Anda yakin ingin menghapus poliklinik ini? Tindakan ini tidak dapat dibatalkan.
              </p>
