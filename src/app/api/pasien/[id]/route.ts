...
export async function GET(
  ...
  try {
    ...
    if (!pasien) {
      return NextResponse.json({ success: false, error: 'Pasien tidak ditemukan' }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: updatedPasien, message: 'Data pasien berhasil diperbarui' });
  } catch (error) {
    return NextResponse.json({ success: false, error: 'Internal Server Error' }, { status: 500 });
  }
}
