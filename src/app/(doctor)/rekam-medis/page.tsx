...
export default function RiwayatPasienPage() {
  ...
  const antreanId = searchParams.get("id"); // Diambil dari URL ?id=...
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        if (jsonAntrean.success && jsonAntrean.data) {
          ...
          // 2. Ambil riwayat rekam medis berdasarkan pasienId
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="md:col-span-1 flex flex-col items-center justify-center border-r border-gray-50 pr-6">
            ...
            <p className="text-xs font-black text-primary uppercase tracking-widest">{pasien.noRm}</p>
          </div>
              ...
              <div key={h.id} className="bg-white border border-gray-100 p-6 rounded-3xl shadow-sm space-y-4 hover:border-primary/30 transition">
                <div className="flex justify-between items-start border-b border-gray-50 pb-3">
                  <div className="flex items-center gap-3">
                    <div className="bg-primary text-white text-[10px] font-black px-3 py-1 rounded-full uppercase">
                      {formatTanggal(h.tanggal)}
                    </div>
                    <span className="text-[10px] font-bold text-gray-400 uppercase">ID: {h.id.split('-')[0]}</span>
                  </div>
                  <div className="text-xs font-black text-gray-400">Dokter: <span className="text-gray-800">{h.dokter?.namaLengkap}</span></div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Bagian Kiri: Diagnosis & Keluhan */}
                  ...
                  {/* Bagian Kanan: Resep & Status Rujukan */}
                  <div className="space-y-3">
                    <div className="bg-blue-50 border border-blue-100 p-4 rounded-2xl">
                      ...
                      <div className="text-sm font-black text-blue-900 leading-relaxed">
                        {Array.isArray(h.resep) && h.resep.length > 0 ? (
                          ...
                          <span className="italic">Tidak ada resep obat</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
                      ...
                      <div className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase ${Array.isArray(h.rujukan) && h.rujukan.length > 0 ? 'bg-orange-100 text-orange-600' : 'bg-green-100 text-green-600'}`}>
