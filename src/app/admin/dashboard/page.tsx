...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    setQueue(appointments.filter((a) => a.status === "antri").sort((a, b) => a.time.localeCompare(b.time)));
    setLoading(false);
  }, [router]);
    ...
    <ProtectedRoute requiredRole="ADMIN">
      <div className="space-y-8">
        <div className="bg-white p-8 rounded-xl shadow-lg border">
          ...
          <p className="text-gray-600">Selamat datang, {session?.username}!</p>
        </div>
        ...
        <div className="bg-white p-8 rounded-xl shadow-lg border">
          ...
          {queue.length === 0 ? (
            ...
            <div className="space-y-4">
              {queue.map((apt) => (
                <div key={apt.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border">
                  <div>
                    <div className="font-bold text-lg">{apt.patientName}</div>
                    <div className="text-sm text-gray-600">{apt.time} - {apt.keluhan || "Umum"}</div>
                  </div>
